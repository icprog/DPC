...
QList<double> floatToDouble(const std::vector<float> &fLst)
{
    QList<double> dLst;
    QVector<float> fList = QVector<float>::fromStdVector(fLst);
    foreach(float f, fLst)
    {
        if (!dLst.contains(f))
        {
            dLst.append(f);
...
double** floatToDouble2D(const std::vector<float> &fDataList, int xLen, int yLen)
{
    double **dData = NULL;
    int count = fDataList.size();
    if (xLen <=0 || yLen <=0 || xLen*yLen!=count)
    {
        return dData;
    }

    dData = new double*[yLen];

    for (int i=0; i<yLen; i++)
    {
        dData[i] = new double[xLen];
        for (int j=0; j<xLen; j++)
        {
            dData[i][j] = fDataList[i*xLen + j];
        }
    }
    return dData;
}
