...
void CPluginManagerImp::initGui(UIInterface *guiInterface)
{
    ///////////////////////////////插件管理start///////////////////////////
    ...
    try
    {
        QList<QSharedPointer<ctkPlugin> >  pluginsList = framework->getPluginContext()->getPlugins();
        ...
        {
            QSharedPointer<ctkPlugin> plugin = pluginsList.at(i);

            QHash<QString, QString> hs = plugin->getHeaders();
            QHash<QString, QString>::iterator iter = hs.find("Export-Interface");

            QSLOG_INFO(QString("init plugins %1 gui interface begin").arg(hs.value("Plugin-Name")));
            ...
            if (iter != hs.end())
            {
                ...
                foreach (const QString &strInterface, strExportInterfaces)
                {
                    ...
                    foreach (const QString &interf, strInterfaceList)
                    {
                        ...
                        if (baseInterface)
                        {
                            baseInterface->initgui(guiInterface);
                            QSLOG_INFO(QString("init gui interface %1 finished").arg(interf));
                        }
                        else
                        {
                            QSLOG_ERROR(tr("init interface:") + strInterface + tr("gui failure"));
                        }
                    }
                }
            }
            ...
            {
                QSLOG_INFO(QString("init plugins %1 gui interface failure").arg(hs.value("Plugin-Name")));
            }
        }
    }
    catch (const ctkPluginException& e)
    {
        QSLOG_ERROR(tr("initgui ctkPluginFramework exception, reason :") + e.what());
    }
